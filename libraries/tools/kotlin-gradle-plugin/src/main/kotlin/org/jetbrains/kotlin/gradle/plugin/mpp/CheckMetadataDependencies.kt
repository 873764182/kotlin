...
open class CheckMetadataDependencies : DefaultTask() {
    ...
    private fun detectIncorrectDependenciesWithoutModuleMetadata(): Set<ResolvedDependency> {
        ...
        return configuration.resolvedConfiguration.lenientConfiguration.firstLevelModuleDependencies
            .filterTo(mutableSetOf()) { resolvedDependency ->
                val moduleFile = resolvedDependency.moduleArtifacts.singleOrNull()?.file
                    ?: return@filterTo false

                val moduleJson =
                    moduleFile.bufferedReader().use { bufferedReader -> Gson().fromJson(bufferedReader, JsonElement::class.java) }
                        .asJsonObjectOrNull
                        ?: return@filterTo false
